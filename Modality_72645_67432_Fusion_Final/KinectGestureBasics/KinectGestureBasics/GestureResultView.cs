...
namespace KinectGestureBasics
{
    ...
    public sealed class GestureResultView : INotifyPropertyChanged
    {
        ...
        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource volUpImage = new BitmapImage(new Uri(@"Images\aumentarVol_Right.png", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource volDownImage = new BitmapImage(new Uri(@"Images\diminuirVol_Right.png", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource closeImage = new BitmapImage(new Uri(@"Images\fechar.jpeg", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource muteImage = new BitmapImage(new Uri(@"Images\mute.jpg", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource repeatImage = new BitmapImage(new Uri(@"Images\repetir.png", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource reproduzirImage = new BitmapImage(new Uri(@"Images\reproduzir.png", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource retrocederImage = new BitmapImage(new Uri(@"Images\SLHretroceder_Left.jpeg", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource avançarImage = new BitmapImage(new Uri(@"Images\SRHavancar_Right.jpeg", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource pauseImage = new BitmapImage(new Uri(@"Images\pause.jpg", UriKind.Relative));

        /// <summary> Image to show when the body associated with the GestureResultView object is not being tracked </summary>
        private readonly ImageSource homeFolderImage = new BitmapImage(new Uri(@"Images\home_folder.jpg", UriKind.Relative));
        ...
        //public void UpdateGestureResult(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence, int gesture_number)
        public void UpdateGestureResult(bool isBodyTrackingIdValid, bool volUpDetected, bool volDownDetected, bool closeDetected, bool muteDetected, float muteConf, bool repetirDetected, bool reproduzirDetected, float reproduzirConf, bool retrocederDetected, float retrocederConf, bool avancarDetected, float avancarConf, bool homeScreenDetected, float homeScreenConf, bool pararDetected, float pararConf)
        {
            ...
            else
            {
                ...
                if (volUpDetected)
                {
                    this.ImageSource = this.volUpImage;
                }
                else if (volDownDetected)
                {
                    this.ImageSource = this.volDownImage;
                }
                else if (closeDetected)
                {
                    this.ImageSource = this.closeImage;
                }
                else if (muteDetected)
                {
                    this.ImageSource = this.muteImage;
                    this.Confidence = muteConf;
                }
                else if (repetirDetected)
                {
                    this.ImageSource = this.repeatImage;
                }
                else if (reproduzirDetected)
                {
                    this.ImageSource = this.reproduzirImage;
                    this.Confidence = reproduzirConf;
                }
                else if (retrocederDetected)
                {
                    this.ImageSource = this.retrocederImage;
                    this.Confidence = retrocederConf;
                }
                else if (avancarDetected)
                {
                    this.ImageSource = this.avançarImage;
                    this.Confidence = avancarConf;
                }
                else if (homeScreenDetected)
                {
                    this.ImageSource = this.homeFolderImage;
                    //this.ImageSource = this.notSeatedImage;
                    this.Confidence = homeScreenConf;
                }
                else if (pararDetected)
                {
                    this.ImageSource = this.pauseImage;
                    //this.ImageSource = this.notSeatedImage;
                    this.Confidence = pararConf;
                }
                else
                {
                    this.ImageSource = this.notSeatedImage;
